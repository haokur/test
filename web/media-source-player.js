class MediaSourcePlayer {
    ...
    constructor(options) {
        ...
        const {
            ...
            type = defaultType,
            onError, onEnd, onUpdateEnd
        } = options
        this.videoUrl = videoUrl
        this.chunkSize = chunkSize
        this.videoType = type
        this.onError = onError || ((err) => { console.log(err) })
    ...
    async onUpdateEnd() {
        if (this.isAllLoaded) { return this.onEnd() }
    ...
    async insertSourceByChunkIndex(chunkIndex) {
        let [start, end] = this.getRangByChunkIndex(chunkIndex)
        let data = await this.fetchSliceData(start, end);
